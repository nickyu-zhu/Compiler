    ...
    if(T){
        switch(T->kind){
            ...
            case ID://检测新的变量名是否唯一
                ...
                if(command==0){//定义变量
                    while(i<new_table.index){
                        if(strcmp(new_table.symbols[i].name,T->type_id)==0 && new_table.symbols[i].flag==flag){ //same name
                            if(flag=='V')
   		...
   		funcOrnot = 1;
            ...
            case FUNC_DEC:
                strcpy(new_table.symbols[new_table.index].name,T->type_id);
                new_table.symbols[new_table.index].level=0; //不允许嵌套函数
                new_table.symbols[new_table.index].type=type;
