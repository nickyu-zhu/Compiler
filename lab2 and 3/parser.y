...
%token PLUS MINUS MUL DIV ASSIGNOP AND OR NOT CALASSIGN SELFOP
...
FuncDec: ID LP VarList RP {$$=mknode(FUNC_DEC,$3,NULL,NULL,yylineno);strcpy($$->type_id,$1);}//函数名存放在$$->type_id
         ...
         ;
/*数组声明*/
ArrayDec: LB INT_LITERAL RB ID {$$=mknode(ARRAY_DEC,$2,NULL,NULL,yylineno);strcpy($$->type_id,$4);}
...
Exp: Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用,正好存放运算符
     ...
     | SELFOP Exp {$$=mknode(SELFOP,$2,NULL,NULL,yylineno);strcpy($$->type_id,"SELFOP");}
     | Exp SELFOP {$$=mknode(SELFOP,$1,NULL,NULL,yylineno);strcpy($$->type_id,"SELFOP");}
     ...
     | CHAR_LITERAL {$$=mknode(CHAR_LITERAL,NULL,NULL,NULL,yylineno); $$->type_char=$1;$$->type=CHAR_LITERAL;}
     ;
/*用逗号隔开的参数*/
